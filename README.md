...
- **A coder release is followed to the end** — after Release, the Changes tab and the chat report how the sandbox deploy it started ended: live (with the version), rolled back, or failed. A failed deploy is handed back to the coder automatically with the failing log lines, asking it to reproduce and fix the build without releasing; you review the fix and release it. Once per released commit, so a build the coder cannot fix never loops. Before this a failed deploy was silent, and the panel said "Deploying" indefinitely.
- **Anonymous daily install check-in (opt-out)** — once a day each AppCrane server sends `https://ping.appcrane.dev/v1/checkin` one small JSON document: a random install id (generated on first boot, stored in `DATA_DIR/install-id`, tied to nothing), the AppCrane version, Node version, OS and CPU architecture, and three counts: registered apps, apps with a live deploy, and active users. No names, domains, emails or app contents, and the receiver does not store IP addresses. It exists to learn how many installs there are and which versions and runtimes they run. Turn it off with `APPCRANE_CHECKIN=off` (or `DO_NOT_TRACK=1`); every boot logs whether it is on and what it sends. `APPCRANE_CHECKIN_URL` points it elsewhere.
- **The database no longer stalls AppCrane when the disk is busy** — SQLite's WAL checkpoint used to run inside whichever request's commit crossed the threshold, fsync included, on the event loop. Under a throttled, contended disk that meant 3.5-5.9 s stalls every few seconds; switching to `synchronous = NORMAL` did not help. The server now checkpoints from a worker thread every 2 s (PASSIVE, so writes are never locked out), and the worst commit measured 1.9-2.4 ms. Every commit is still flushed to disk as before, so nothing is less durable. The `crane` CLI keeps SQLite's default. MariaDB setup also checks that the root password on record opens the server before using it, and refuses a server it did not start with a message naming the container, the likely cause and the fix, instead of a bare "Access denied" later.
- **The Coder button lives on the Sandbox tab** — it is shown while you review the sandbox, not on production, and switching to Production closes the panel (a running turn carries on and is there when you return). Five tests that failed now and then on CI were traced and fixed in the tests themselves; timing checks that must not share a disk with the parallel suite live in `test/serial/` and run as their own CI step (`npm run test:serial`).
- **Self-update brings its own Node when it cannot install one** — a host below the Node floor used to refuse every update unless AppCrane ran as root, because upgrading Node meant `apt-get`. Now, when the system package manager is out of reach (not root, no apt, or an nvm/asdf Node), self-update downloads the official Node build from nodejs.org into `<appcrane>/.runtime/`, checks the GPG signature on the release's `SHASUMS256.txt` with `gpgv` against Node's release keys (shipped as `infra/node-release-keys.gpg`, refreshed by `scripts/update-node-release-keys.sh` from `nodejs/release-keys`), checks the tarball's SHA-256 against that signed list, runs it to confirm the major and that it works on this host's libc, and uses it for the rest of the update. A host with no `gpgv` (it comes with `apt`) is refused unless `APPCRANE_NODE_ALLOW_UNSIGNED=1` accepts the checksum alone. `scripts/safe-boot.sh` puts it first on PATH at every start while the system Node is below the floor, and steps aside once the system Node meets it. A host already stuck below the floor is repaired the same way at its next boot, without root. It needs the service to start through `scripts/safe-boot.sh` (install.sh's unit does) and Linux on x64 or arm64; otherwise the update still refuses, and says which of the two is missing.
- **Coder modes, and files in the chat** — each message picks a mode beside the model: **Auto** (reads, edits and runs commands without asking — what the coder always did), **Edits only** (edits files, shell commands are refused) or **Plan** (reads the code and proposes a plan, changes nothing). Each maps to one Claude Code `--permission-mode`; the list is served by `GET /api/coder/models` and enforced as exact strings at the route and again where the command is built. A message can also carry files: paste a screenshot or a copied file, drop it on the composer, or use Attach (10 MB each, 10 per message). They are uploaded to `POST /api/coder/:slug/session/:id/attachments`, stored per session on the host under an opaque id — never in the workspace, so they are never listed as changes or released — copied into the container when the turn runs, and named to the agent by path, where Claude Code's Read tool reads text, code, images and PDFs. A queued follow-up keeps its own mode and files. Per-action approval from the panel (a Manual mode) is not built yet.
- **The coder can run shell commands and keep its conversation** — inside its container the coder runs as the `studio` user, but `~/.claude` was created by Docker as root (the transcripts are mounted beneath it) and every container start handed the saved conversation to a different user. Claude could not create the state its Bash tool needs, so the agent reported "Bash is blocked by an environment permission issue", and it could neither read nor extend its own conversation after a restart. The studio image now creates `~/.claude` for `studio` with a pinned uid/gid, and the transcripts are owned by that user, repaired on the next start. The image version is bumped, so each host rebuilds it once, on the next coder start.
- **Messages from the element picker reach the coder** — a message that starts with a dash, which every picker message does (`--- Pointed element ---`), was parsed by the Claude CLI as a command-line option and failed with `unknown option`. The prompt is now passed after `--`, so no message can be read as a flag. The codebase summary built before a first message now reads the coder's workspace (host git had refused it as owned by another user, leaving the summary with no commit and an empty file tree), runs on the same credential as your turn, and when it fails says what Claude said rather than only an exit code.
- **A message sent to a paused coder runs instead of vanishing** — if the coder's container was stopped (an eviction, or an AppCrane restart the open page never heard about), sending a message used to fail with an error while the panel still said idle. The panel now resumes the session and runs the message; a failed resume says the message was not run and why. When Claude rejects the credential, the turn stops at the first 401 instead of waiting out about three minutes of retries, and the error names the credential to replace: your subscription token, the app's uploaded credentials, or the platform's `ANTHROPIC_API_KEY`. The codebase summary built before a first message now says it is working, says so if it fails, and is skipped for a conversation that is only being resumed.
- **A coder that is starting says so, and one that failed says why** — resuming a paused session recreates its container, and the first time after an upgrade it rebuilds the agent image, which takes a minute or more. That used to show nothing at all, so a slow resume looked exactly like a broken button. It now shows a running counter and what is happening, gives up visibly after ten minutes rather than waiting forever, and when the image build fails the message carries the last lines of build output — an unreachable npm registry and a full disk no longer read identically as `exit 1`
- **The Coder button tells you what it would take** — it appears on every app, not only the ones it works on. Where it cannot run yet it is muted, and opening it lists every reason at once with its fix: the app's code lives on GitHub, it is still an uploaded archive, it runs from an image, its repository was not migrated, or you have not connected a Claude account (with a link to where you do). The list comes from the same checks that would refuse a session, so it cannot promise something the server will then deny, and no session is started only to be refused
- **What you teach one agent, the next one knows** — an app carries operator notes that are prepended to every coder dispatch. Your local Claude Code can now write them over MCP (`appcrane_append_app_context`), so a constraint or a gotcha discovered on your machine reaches the in-platform coder instead of dying with the session. Append is the default because it cannot destroy what a human wrote; the replace tool reports how much it discarded, and the notes are capped, because anything prepended to every dispatch silently eats the context window it was meant to help
- **The coder actually has a shell** — the agent image is Alpine-based, which ships busybox `sh` and no bash, and Claude Code's Bash tool spawns bash specifically. Every shell call the agent made failed at spawn, so it could read and edit files but reported having "no shell" and handed the user commands to run by hand — a session that looked healthy while builds, tests and git were quietly unavailable. The image installs bash, and its version is bumped so existing hosts rebuild
- **Builds survive a Docker install without buildx** — BuildKit needs the `docker buildx` plugin, and Ubuntu's `docker.io` package ships the daemon without it, so forcing BuildKit on made every deploy fail at the build step with a message that pointed at Docker's docs rather than the fix. AppCrane now probes once, uses BuildKit when it is there and the classic builder when it is not, saying so in the log. `APPCRANE_DOCKER_BUILDKIT` pins the choice in either direction
...
- **AppStudio** — turns a feature request into a plan you approve. For a GitHub-backed app it then writes the code and puts it on sandbox for your review before anything reaches production; for a Crane-hosted app it analyzes the repository and files the result as a new request, and the coder makes the change
...
> `claude setup-token` (Pro, Max, Team or Enterprise plan) and saves it under
> Settings → Account, which strips the whitespace a terminal copy picks up and
> says what it removed; nobody else, platform admins included, can read, replace or clear
...
│   ├── Health checker (cron)
│   ├── SSO (SAML / OIDC / SCIM)
│   ├── AppStudio (request → plan → code) + coder
│   └── Presence (WebSocket)
└── /data/apps/myapp/
