...
- **Managed-app repositories can live on the AppCrane host** — groundwork for moving managed apps off GitHub: bare git repositories under `DATA_DIR/repos`, written with git plumbing (no worktree), isolated from the host's git configuration and hooks, with compare-and-swap on every push so a concurrent push fails instead of silently overwriting another. They are in the config backup from the first commit — rebuilt and `git fsck --full`-checked in a staging directory before anything live is replaced, so a corrupt bundle refuses the whole import. Not yet used by any app; managed apps still deploy from GitHub until the platform is switched over
