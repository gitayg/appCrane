...
- **Managed-app repositories can live on the AppCrane host** — groundwork for moving managed apps off GitHub: bare git repositories under `DATA_DIR/repos`, written with git plumbing (no worktree), isolated from the host's git configuration and hooks, and when two pushes race the newest wins: the push that landed first is not overwritten silently — it is parked under a `refs/dropped/` ref, reported back to the pusher, and carried in backups, so it can be recovered. They are in the config backup from the first commit — rebuilt and `git fsck --full`-checked in a staging directory before anything live is replaced, so a corrupt bundle refuses the whole import. Not yet used by any app; managed apps still deploy from GitHub until the platform is switched over
- **Restore onto a bare host, with no registry** — the config backup carries the database, secrets, icons, per-app `/data` and every declared volume. A second, streamed archive carries the container images themselves, saved by digest. That matters because a registry is not a backup: `bitnami/*` images 404 after their registry change, and `medusajs/medusa`, `vendureio/vendure` and `crater/crater` all 404 today. When a pull fails, a deploy now falls back to bytes already on the host instead of refusing to start
