...
| `fromName` | no | Sender display name. **Defaults to the app's own name** — MarketMind sends as `MarketMind <[redacted][redacted].com>`. Pass this to override per-send (e.g. `"IntelOP"`). The address never changes. |
...
- **The address is platform-controlled; the display name is the app's.** From
  address is fixed (the Settings → Mail mailbox, e.g. `[redacted][redacted].com`). The
