...
address). Mail goes out as the platform sender (e.g. `AIMI <[redacted][redacted].com>`)
...
await fetch(`${process.env.CRANE_INTERNAL_URL}/api/service/email`, {
  ...
  body: JSON.stringify({
    ...
    replyTo: 'team@[redacted].com',     // optional
...
- **From identity is platform-controlled.** Address is fixed
  (`[redacted][redacted].com`); only the display name is configurable (per-app via
...
e.g. `app.[redacted].com` → any `*.[redacted].com`) embed apps — including the in-iframe
...
domain and scheme (e.g. `https://portal.[redacted].com` embedding
`https://app.[redacted].com` — same `[redacted].com`, both https → the SSO session flows
