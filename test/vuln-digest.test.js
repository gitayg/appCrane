...
const alice = mkUser('Alice', 'alice@[redacted].com', 'user');
const bob   = mkUser('Bob',   'bob@[redacted].com',   'user');
const carol = mkUser('Carol', 'carol@[redacted].com', 'user');
const root  = mkUser('Root',  'root@[redacted].com',  'platform_admin');
...
test('an app owner is scoped to their own apps; a platform admin gets the fleet', () => {
  ...
  assert.deepEqual([...byEmail.keys()].sort(),
    ['alice@[redacted].com', 'bob@[redacted].com', 'carol@[redacted].com', 'root@[redacted].com']);

  assert.equal(byEmail.get('root@[redacted].com').scope, 'platform');
  assert.deepEqual(byEmail.get('root@[redacted].com').apps.map(a => a.slug).sort(),
    ['billing', 'intranet', 'telemetry']);

  assert.equal(byEmail.get('alice@[redacted].com').scope, 'owner');
  assert.deepEqual(byEmail.get('alice@[redacted].com').apps.map(a => a.slug), ['billing']);
  assert.deepEqual(byEmail.get('bob@[redacted].com').apps.map(a => a.slug), ['intranet']);
  assert.deepEqual(byEmail.get('carol@[redacted].com').apps.map(a => a.slug), ['telemetry']);
});
...
test('the mail names the app, the package, the ecosystem, the advisory and the fix', () => {
  ...
  const a = rowFor('alice@[redacted].com');
  ...
  const r = rowFor('root@[redacted].com');
  ...
  assert.equal(a.idempotency_key, `vuln-digest:${today}:alice@[redacted].com`);
...
test('one scan can mix ecosystems, and every line names its own', () => {
  const c = rowFor('carol@[redacted].com');
